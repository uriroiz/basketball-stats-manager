/**
 * IBBA Insights V2 - ×ž×¢×¨×›×ª Insights ×ž×ª×§×“×ž×ª ×œ×©×“×¨× ×™ ×›×“×•×¨×¡×œ
 ...
 * 
 * ×”×¢×¨×•×ª:
 * - ×©×ž×•×ª ×©×—×§× ×™×: ×‘×’×œ×œ Pure API, ××™×Ÿ ×’×™×©×” ×™×©×™×¨×” ×œ×©×ž×•×ª ×©×—×§× ×™×.
 *   ×›×¨×’×¢ ×ž×•×¦×’×™× ×ž×¡×¤×¨×™ ×—×•×œ×¦×•×ª (#7, #12 ×•×›×•').
 *   ××¤×©×¨ ×œ×©×œ×‘ ×¢× ibba_player_sync.js ××• playerNamesLoader ×œ×¢×ª×™×“.
 * - ×›×œ ×”×—×™×©×•×‘×™× ×ž×‘×•×¡×¡×™× ×¢×œ × ×ª×•× ×™× ××ž×™×ª×™×™× ×‘×œ×‘×“ - ××™×Ÿ ×”×¢×¨×›×•×ª!
 * - ×¡×£ ×ž×™× ×™×ž×œ×™: ×¨×•×‘ ×”-Insights ×“×•×¨×©×™× ×œ×¤×—×•×ª 3-5 ×ž×©×—×§×™× ×œ×ž× ×™×¢×ª False Positives
 * 
 * ×’×¨×¡×” 2.2.0 (×—×“×©):
 * - ×ž×¢×¨×›×ª ×˜×ž×¤×œ×˜×™× ×“×™× ×ž×™×ª ×¢× 8 ×•×¨×™××¦×™×•×ª ×œ×›×œ Insight
 * - ×œ×ž× ×•×¢ ×—×–×¨×ª×™×•×ª ×•×”×¤×™×›×ª ×”×ž×¢×¨×›×ª ×œ×˜×‘×¢×™×ª ×™×•×ª×¨
 * 
 * ×’×¨×¡×” 2.1.0 (× ×•×¡×£):
 * - Fast Break Kings: 15+ × ×§' ×œ×ž×©×—×§ ×ž×”×ª×§×¤×•×ª ×ž×ª×¤×¨×¦×•×ª
 * - Paint Dominance: 45%+ ×ž×”× ×§×•×“×•×ª ×ž×”×¦×‘×¢
 * - Bench Power: 30+ × ×§' ×œ×ž×©×—×§ ×ž×”×¡×¤×¡×œ
 * - Turnover Capitalization: 18+ × ×§' ×œ×ž×©×—×§ ×ž××™×‘×•×“×™×
 * - Second Chance Masters: 15+ × ×§' ×œ×ž×©×—×§ ×ž×”×–×“×ž× ×•×ª ×©× ×™×™×”
 * 
...
class IBBAInsightsV2 {
  constructor(analytics) {
    ...
    this.playerNamesMap = null; // ××•×¤×¦×™×•× ×œ×™ - ×× ×™×© ×ž×™×¤×•×™ ×©×ž×•×ª ×©×—×§× ×™×
  }
  ...
  /**
   * ×—×™×©×•×‘ ×ž×ž×•×¦×¢ ×œ×™×’×ª×™ ×œ×§×˜×’×•×¨×™×” ×¡×¤×¦×™×¤×™×ª
   * @param {string} metric - ×©× ×”×ž×˜×¨×™×§×” (ppg, rpg, apg, etc.)
   * @param {Array} allTeamsData - × ×ª×•× ×™ ×›×œ ×”×§×‘×•×¦×•×ª
   * @returns {number} ×ž×ž×•×¦×¢ ×œ×™×’×ª×™
   */
   ...
   * @param {boolean} ascending - true ×× × ×ž×•×š ×™×•×ª×¨ = ×˜×•×‘ ×™×•×ª×¨ (×›×ž×• oppPpg)
   * @returns {number} ×“×™×¨×•×’ (1 = ×”×›×™ ×˜×•×‘)
   */
  ...
  detectClutchStreak(teamName, games, rank = null) {
    ...
    if (clutchWins >= 2) {
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('streaks', 'CLUTCH_WINS', {
        ...
        clutchWins
      }) || `${teamWithRank} ×‘×¨×¦×£ ×©×œ ${clutchWins} × ×™×¦×—×•× ×•×ª ×¦×ž×•×“×™× (×”×¤×¨×© ×¢×“ 5 × ×§') â€“ ×ž×’×™×¢×” ×¢× ×™×›×•×œ×ª ×œ× ×¦×— ×‘×ž×©×—×§×™× ×§×¨×•×‘×™×`;
  ...
  detectLosingStreak(teamName, games, rank = null) {
    ...
    if (losses >= 3) {
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('streaks', 'LOSING_STREAK', {
        teamName: teamWithRank,
        losses
      }) || `${teamWithRank} ×‘×¨×¦×£ ×©×œ ${losses} ×”×¤×¡×“×™× â€“ ×ž×’×™×¢×” ×¢× × ×§×•×“×ª ×©××œ×” ×œ×§×¨××ª ×”×ž×©×—×§`;
  ...
  detectBlowoutWins(teamName, games, rank = null) {
    ...
    if (blowouts >= MIN_BLOWOUTS) {
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('streaks', 'BLOWOUT_WINS', {
        teamName: teamWithRank,
        blowouts
      }) || `${teamWithRank} ×¢× ${blowouts} × ×™×¦×—×•× ×•×ª ×’×“×•×œ×™× (15+ × ×§') ×‘-5 ×ž×©×—×§×™× ××—×¨×•× ×™× â€“ ×ž×’×™×¢×” ×¢× ×“×•×ž×™× × ×˜×™×•×ª ×‘×•×œ×˜×ª`;
  ...
  detectCloseLosses(teamName, games) {
    ...
    if (closeLosses >= MIN_CLOSE_LOSSES) {
      return {
        ...
        value: closeLosses,
        icon: 'ðŸ˜¤',
        text: `${teamName} ×¢× ${closeLosses} ×”×¤×¡×“×™× ×¦×ž×•×“×™× ×‘×¢×•× ×” - ×§×¨×•×‘×” ×œ×¤×¨×™×¦×”! ×¨×§ ×¦×¨×™×›×” ×ž×–×œ ×§×˜×Ÿ`,
        textShort: `${closeLosses} ×”×¤×¡×“×™× ×¦×ž×•×“×™× ×‘×¢×•× ×”`
      };
    }
    return null;
  }
  ...
  /**
   * ×–×™×”×•×™ ×©×—×§×Ÿ "×‘×•×¢×¨" (Hot Hand)
   * ×©×—×§×Ÿ ×©×§×•×œ×¢ ×ž×¢×œ 150% ×ž×”×ž×ž×•×¦×¢ ×©×œ×• ×‘-3 ×ž×©×—×§×™× ××—×¨×•× ×™×
   */
  detectHotHand(teamName, games) {
    const MIN_GAMES = 5; // ×ž×™× ×™×ž×•× ×ž×©×—×§×™× ×œ×—×™×©×•×‘ ×ž×ž×•×¦×¢ ×¢×•× ×ª×™
    ...
    for (const [playerId, data] of Object.entries(playerGames)) {
      ...
      // ×‘×“×™×§×ª ×¡×£
      if (recentAvg >= seasonAvg * THRESHOLD && seasonAvg >= 8) { // ×ž×™× ×™×ž×•× 8 × ×§' ×‘×ž×ž×•×¦×¢
        ...
        return {
          ...
          percentAbove,
          icon: 'ðŸ”¥',
          text: `${playerName} ×©×œ ${teamName} ×‘×•×¢×¨! ${recentAvg.toFixed(1)} × ×§' ×‘-3 ×ž×©×—×§×™× ××—×¨×•× ×™× (×œ×¢×•×ž×ª ${seasonAvg.toFixed(1)} ×¢×•× ×ª×™) - +${percentAbove}%`,
    ...
    for (const [playerId, data] of Object.entries(playerGames)) {
      ...
      if (recentAvg <= seasonAvg * THRESHOLD && seasonAvg >= 10) {
        ...
        return {
          ...
          percentBelow,
          icon: 'â„ï¸',
          text: `${playerName} ×©×œ ${teamName} ×‘×ž×©×‘×¨ - ×¨×§ ${recentAvg.toFixed(1)} × ×§' ×‘-3 ×ž×©×—×§×™× ××—×¨×•× ×™× (×œ×¢×•×ž×ª ${seasonAvg.toFixed(1)} ×¢×•× ×ª×™) - ×™×¨×™×“×” ×©×œ ${percentBelow}%`,
          textShort: `${playerName} ×‘×ž×©×‘×¨ (${recentAvg.toFixed(1)} vs ${seasonAvg.toFixed(1)})`
        };
      }
    }
    ...
    for (const [playerId, data] of Object.entries(playerGames)) {
      ...
      if (h2hAvg >= seasonAvg * THRESHOLD && seasonAvg >= 8) {
        ...
        return {
          ...
          percentAbove,
          icon: 'ðŸŽ¯',
          text: `${playerName} ×©×œ ${teamName} = ×”×¨×•×¦×— ×©×œ ${opponentName}! ×ž×ž×•×¦×¢ ×©×œ ${h2hAvg.toFixed(1)} × ×§' ×‘×ž×¤×’×©×™× (×œ×¢×•×ž×ª ${seasonAvg.toFixed(1)} ×¢×•× ×ª×™) - +${percentAbove}%`,
    ...
    for (const [playerId, data] of Object.entries(playerStats)) {
      ...
      if (apg >= MIN_APG) {
        ...
        const text = window.IBBAInsightTemplates?.getRandomText('player', 'ASSIST_MACHINE', {
          playerName,
          apg: apg.toFixed(1)
        }) || `${playerName} ×ž×’×™×¢ ×›×ž×¤×™×¥ ×‘×•×œ×˜ â€“ ${apg.toFixed(1)} ××¡×™×¡×˜×™× ×œ×ž×©×—×§ ×‘×¢×•× ×”`;
        ...
        return {
          ...
          icon: 'ðŸŽ¯',
          text,
          textShort: `${playerName}: ${apg.toFixed(1)} ××¡×™×¡×˜×™× ×œ×ž×©×—×§`
        };
      }
    }
    ...
    for (const [playerId, data] of Object.entries(playerStats)) {
      ...
      if (rpg >= MIN_RPG) {
        ...
        const text = window.IBBAInsightTemplates?.getRandomText('player', 'REBOUND_MACHINE', {
          playerName,
          rpg: rpg.toFixed(1)
        }) || `${playerName} ×ž×’×™×¢ ×›××—×“ ×”×¨×™×‘××•× ×“×¨×™× ×”×‘×•×œ×˜×™× â€“ ${rpg.toFixed(1)} ×›×“×•×¨×™× ×—×•×–×¨×™× ×œ×ž×©×—×§ ×‘×¢×•× ×”`;
        ...
        return {
          ...
          icon: 'ðŸ€',
          text,
          textShort: `${playerName}: ${rpg.toFixed(1)} ×¨×™×‘××•× ×“×™× ×œ×ž×©×—×§`
        };
      }
    }
  ...
  detectTeamLeader(teamName, games, teamData) {
    ...
    if (leader && maxPpg >= 15) {
      ...
      const pctText = playerPct > 0 ? ` (${playerPct}% ×ž× ×§×•×“×•×ª ×”×§×‘×•×¦×”)` : '';
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('player', 'TEAM_LEADER', {
        ...
        pctText
      }) || `${playerName} ×ž×•×‘×™×œ ××ª ${teamName} ×¢× ${maxPpg.toFixed(1)} × ×§' ×œ×ž×©×—×§${pctText} â€“ ×”×›×ª×•×‘×ª ×”×¨××©×•× ×” ×‘×”×ª×§×¤×”`;
      ...
      return {
        ...
        icon: 'ðŸ‘‘',
        text,
        textShort: `${playerName}: ×ž×•×‘×™×œ ×¢× ${maxPpg.toFixed(1)} × ×§'`
      };
    }
    ...
    for (const [playerId, data] of Object.entries(playerStats)) {
      ...
      if (ddPct >= THRESHOLD) {
        ...
        const text = window.IBBAInsightTemplates?.getRandomText('player', 'DOUBLE_DOUBLE_MACHINE', {
          ...
          games: data.games
        }) || `${playerName} ×ž×’×™×¢ ×¢× ×¢×§×‘×™×•×ª ×—×¨×™×’×”: ${data.doubleDoubles} ×“××‘×œ-×“××‘×œ×™× ×‘-${data.games} ×ž×©×—×§×™× ×‘×¢×•× ×”`;
  ...
  detectMrConsistent(teamName, games) {
    const MIN_GAMES = 7;
    const MAX_CV = 25; // Coefficient of Variation ×ž×§×¡×™×ž×œ×™
    const MIN_AVG = 10; // × ×§×•×“×•×ª ×ž×™× ×™×ž×•× ×‘×ž×ž×•×¦×¢
    ...
    if (mostConsistent) {
      ...
      return {
        ...
        playerName,
        icon: 'ðŸ“Š',
        text: `${playerName} ×©×œ ${teamName} = ×¢×§×‘×™×•×ª ×ž×•×—×œ×˜×ª! ${mostConsistent.mean.toFixed(1)} × ×§' ×‘×ž×ž×•×¦×¢ ×¢× ×¡×˜×™×™×ª ×ª×§×Ÿ ${mostConsistent.stdDev.toFixed(1)} ×‘×œ×‘×“`,
  ...
  detectBoomOrBust(teamName, games) {
    const MIN_GAMES = 7;
    const MIN_CV = 40; // Coefficient of Variation ×ž×™× ×™×ž×œ×™
    const MIN_AVG = 10;
    ...
    if (mostVolatile) {
      ...
      return {
        ...
        playerName,
        icon: 'ðŸŽ¢',
        text: `${playerName} ×©×œ ${teamName} = ×œ× ×¦×¤×•×™! ${mostVolatile.mean.toFixed(1)} × ×§' ×‘×ž×ž×•×¦×¢ ××‘×œ ×¡×˜×™×™×ª ×ª×§×Ÿ ${mostVolatile.stdDev.toFixed(1)} - ×’×‘×•×” ××• × ×ž×•×š`,
  ...
  detectHomeCourtHero(teamName, games) {
    ...
    if (hero) {
      ...
      return {
        ...
        playerName,
        icon: 'ðŸ ',
        text: `${playerName} ×©×œ ${teamName} ××•×”×‘ ××ª ×”×‘×™×ª! ${hero.homePpg.toFixed(1)} × ×§' ×‘×‘×™×ª ×œ×¢×•×ž×ª ${hero.awayPpg.toFixed(1)} ×‘×—×•×¥ (+${hero.diff.toFixed(1)})`,
  ...
  detectRisingStar(teamName, games) {
    ...
    if (star) {
      ...
      return {
        ...
        playerName,
        icon: 'ðŸ“ˆ',
        text: `${playerName} ×©×œ ${teamName} ×‘×ž×’×ž×ª ×¢×œ×™×™×”! ${star.firstAvg.toFixed(1)} × ×§' ×‘×ª×—×™×œ×” â†’ ${star.secondAvg.toFixed(1)} × ×§' ×œ××—×¨×•× ×” (+${star.improvementPct.toFixed(0)}%)`,
  ...
  detectThreePointDependent(teamName, teamData, allTeams) {
    ...
    if (threePointPct >= THRESHOLD) {
      ...
      const rankText = rank ? ` (×ž×§×•× ${rank} ×‘×œ×™×’×” ×‘×ª×œ×•×ª ×‘×©×œ×•×©)` : '';
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('team', 'THREE_POINT_DEPENDENT', {
        ...
        teamName,
        rankText
      }) || `${threePointPct.toFixed(0)}% ×ž× ×§×•×“×•×ª ${teamName}${rankText} ×ž×§×• ×”×©×œ×•×© â€“ ×ª×œ×•×ª ×’×‘×•×”×” ×‘×ž×©×—×§ ×”×ž×¨×—×§`;
      ...
      return {
        ...
        icon: 'ðŸŽ¯',
        text,
        textShort: `${threePointPct.toFixed(0)}% ×ž×”× ×§×•×“×•×ª ×ž×©×œ×•×©`
      };
    }
    return null;
  }
  ...
  detectPaintDominators(teamName, teamData, leagueAvg) {
    ...
    if (twoPointPpg >= leagueTwoPpg + 8) {
      return {
        ...
        icon: 'ðŸ€',
        text: `${teamName} ×©×•×œ×˜×ª ×‘×¦×‘×¢ - ${twoPointPpg.toFixed(1)} × ×§' ×œ×ž×©×—×§ ×ž×–×¨×™×§×•×ª ×©×œ 2 (${twoPointFGM} ×¡×œ×™× ×œ×ž×©×—×§, ${(twoPointPpg - leagueTwoPpg).toFixed(1)} ×ž×¢×œ ×ž×ž×•×¦×¢ ×”×œ×™×’×”)`,
        textShort: `${twoPointPpg.toFixed(1)} × ×§' ×ž×–×¨×™×§×•×ª 2`
      };
    }
    return null;
  }
  ...
  detectAssistHeavy(teamName, teamData, allTeams) {
    ...
    if (assistRatio >= THRESHOLD) {
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('team', 'ASSIST_HEAVY', {
        ...
        teamName,
        rankText
      }) || `${assistRatio.toFixed(1)}% ×ž×”×¡×œ×™× ×©×œ ${teamName}${rankText} ×ž×’×™×¢×™× ×ž××¡×™×¡×˜ â€“ ×ž×©×—×§ ×§×‘×•×¦×ª×™ ×‘×•×œ×˜`;
      ...
      return {
        ...
        icon: 'ðŸ¤',
        text,
        textShort: `${assistRatio.toFixed(1)}% ×¡×œ×™× ×ž××¡×™×¡×˜`
      };
    }
    return null;
  }
  ...
  detectFreeThrowFactory(teamName, teamData) {
    ...
    if (ftaPerGame >= THRESHOLD) {
      return {
        ...
        icon: 'ðŸŽ¯',
        text: `${teamName} ×ž×’×™×¢×” ×”×¨×‘×” ×œ×§×• ×”×—×™× × â€“ ${ftaPerGame.toFixed(1)} ×–×¨×™×§×•×ª ×—×•×¤×©×™×•×ª ×œ×ž×©×—×§ ×‘×ž×ž×•×¦×¢`,
        textShort: `${ftaPerGame.toFixed(1)} ×–×¨×™×§×•×ª ×—×•×¤×©×™×•×ª ×œ×ž×©×—×§`
      };
    }
    return null;
  }
  ...
  detectHighScoringOffense(teamName, teamData, leagueAvgPpg, allTeams) {
    ...
    if (ppg >= leagueAvgPpg + THRESHOLD_DIFF) {
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('team', 'HIGH_SCORING_OFFENSE', {
        ...
        ppg: ppg.toFixed(1),
        diff
      }) || `${teamName}${rankText} ×ž×’×™×¢×” ×¢× ××—×ª ×”×”×ª×§×¤×•×ª ×”×¤×•×¨×™×•×ª ×‘×œ×™×’×” â€“ ${ppg.toFixed(1)} × ×§' ×œ×ž×©×—×§, ${diff} ×ž×¢×œ ×”×ž×ž×•×¦×¢`;
  ...
  /**
   * ×–×™×”×•×™ Fast Break Kings - ×ž×œ×›×™ ×”×”×ª×§×¤×” ×”×ž×ª×¤×¨×¦×ª
   */
  detectFastBreakKings(teamName, teamData, allTeams) {
    const THRESHOLD = 15; // 15 × ×§' ×œ×ž×©×—×§
    const MAX_RANK = 6; // ×¨×§ ×—×¦×™ ×¢×œ×™×•×Ÿ (×ž×§×•×ž×•×ª 1-6 ×ž×ª×•×š ~12-16 ×§×‘×•×¦×•×ª)
    ...
    if (fastBreakPpg >= THRESHOLD) {
      ...
      const rankText = rank ? ` (×ž×§×•× ${rank} ×‘×œ×™×’×” ×‘×”×ª×§×¤×•×ª ×ž×ª×¤×¨×¦×•×ª)` : '';
      ...
      }) || `${teamName}${rankText} ×ž×’×™×¢×” ×›×§×‘×•×¦×” ×ž×”×˜×•×‘×•×ª ×‘×ž×ª×¤×¨×¦×•×ª â€“ ${fastBreakPpg.toFixed(1)} × ×§' ×œ×ž×©×—×§ ×ž×”×ª×§×¤×•×ª ×ž×”×™×¨×•×ª`;
      ...
      return {
        ...
        icon: 'âš¡',
        text,
        textShort: `${fastBreakPpg.toFixed(1)} × ×§' ×”×ª×§×¤×•×ª ×ž×ª×¤×¨×¦×•×ª`
      };
    }
    return null;
  }
  ...
  detectPaintDominance(teamName, teamData, allTeams) {
    ...
    if (paintPct >= THRESHOLD) {
      ...
      const rankText = rank ? ` (×ž×§×•× ${rank} ×‘×œ×™×’×” ×‘× ×§×•×“×•×ª ×‘×¦×‘×¢)` : '';
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('team', 'PAINT_DOMINANCE', {
        ...
        paintPpg
      }) || `${teamName}${rankText} ×ž×’×™×¢×” ×¢× ×›×•×— ×‘×¦×‘×¢ â€“ ${paintPct.toFixed(0)}% ×ž×”× ×§×•×“×•×ª ×©×œ×” ×ž×©×, ${paintPpg} × ×§' ×œ×ž×©×—×§`;
      ...
      return {
        ...
        icon: 'ðŸŽ¯',
        text,
        textShort: `${paintPct.toFixed(0)}% × ×§' ×ž×”×¦×‘×¢`
      };
    }
    return null;
  }
  ...
  detectBenchPower(teamName, teamData, allTeams) {
    ...
    if (benchPpg >= THRESHOLD) {
      ...
      const rankText = rank ? ` (×ž×§×•× ${rank} ×‘×œ×™×’×” ×‘×ª×¨×•×ž×ª ×¡×¤×¡×œ)` : '';
      ...
      return {
        ...
        rank,
        icon: 'ðŸª‘',
        text: `${teamName}${rankText} - ${actionText}! ${benchPpg} × ×§' ×œ×ž×©×—×§ ×ž×”×¡×¤×¡×œ (${benchPct}% ×ž×”× ×§×•×“×•×ª)`,
        textShort: `${benchPpg} × ×§' ×ž×”×¡×¤×¡×œ`
      };
    }
    return null;
  }
  ...
  detectWorstCategory(teamName, teamData, allTeams) {
    const categories = {
      ppg: { label: '×‘× ×™×§×•×“', icon: 'ðŸŽ¯', ascending: false },
      rpg: { label: '×‘×¨×™×‘××•× ×“×™×', icon: 'ðŸ€', ascending: false },
      apg: { label: '×‘××¡×™×¡×˜×™×', icon: 'ðŸ¤', ascending: false },
      oppPpg: { label: '×‘×”×’× ×”', icon: 'ðŸ›¡ï¸', ascending: true }
    };
    ...
    if (worstCategory && worstRank >= 10) {
      return {
        ...
        teamName,
        icon: 'âš ï¸',
        text: `${teamName}: ××ª×’×¨ ×¢×™×§×¨×™ - ×ž×§×•× ${worstCategory.rank} ${worstCategory.label} (${worstCategory.value})`,
        textShort: `××ª×’×¨: ×ž×§×•× ${worstCategory.rank} ${worstCategory.label}`,
        isFallback: true
      };
    }
  ...
  detectBestCategory(teamName, teamData, allTeams) {
    const categories = {
      ...
      rpg: { label: '×‘×¨×™×‘××•× ×“×™×', icon: 'ðŸ€', ascending: false },
      apg: { label: '×‘××¡×™×¡×˜×™×', icon: 'ðŸ¤', ascending: false },
      spg: { label: '×‘×—×˜×™×¤×•×ª', icon: 'ðŸƒ', ascending: false },
      fgPct: { label: '×‘××—×•×– ×§×œ×™×¢×”', icon: 'ðŸŽ¯', ascending: false },
      fg3Pct: { label: '×‘×©×œ×•×©', icon: 'ðŸŽ¯', ascending: false },
      oppPpg: { label: '×‘×”×’× ×”', icon: 'ðŸ›¡ï¸', ascending: true }
    };
    ...
    if (bestCategory) {
      let text;
      if (bestRank <= 3) {
        text = `${teamName}: × ×§×•×“×ª ×—×•×–×§ - ×ž×§×•× ${bestCategory.rank} ${bestCategory.label} (${bestCategory.value})`;
      } else if (bestRank <= 8) {
        text = `${teamName}: × ×§×•×“×ª ×—×•×–×§ ×™×—×¡×™×ª - ×ž×§×•× ${bestCategory.rank} ${bestCategory.label} (${bestCategory.value})`;
      } else {
        text = `${teamName}: ×”×§×˜×’×•×¨×™×” ×”×˜×•×‘×” ×‘×™×•×ª×¨ - ×ž×§×•× ${bestCategory.rank} ${bestCategory.label} (${bestCategory.value})`;
      }
      ...
      return {
        ...
        text,
        textShort: `×ž×§×•× ${bestCategory.rank} ${bestCategory.label}`,
        isFallback: true // ×¡×™×ž×•×Ÿ ×©×–×” fallback
      };
    }
  ...
  /**
   * ×–×™×”×•×™ Starting vs Bench - ×”×©×•×•××ª ×—×ž×™×©×™×™×” ×ž×ª×—×™×œ×” ×œ×¡×¤×¡×œ
   */
  detectStartingVsBench(teamName, teamData, allTeams) {
    ...
    // Insight ×ž×¢× ×™×™×Ÿ ×¨×§ ×× ×™×© ××™×–×•×Ÿ ××• ×¡×¤×¡×œ ×—×–×§ ×‘×ž×™×•×—×“
    const MIN_BENCH_PCT = 25; // ×œ×¤×—×•×ª 25% ×ž×”×¡×¤×¡×œ
    const STRONG_BENCH_PCT = 35; // 35%+ = ×¡×¤×¡×œ ×—×–×§ ×ž××•×“
    ...
    if (benchPct >= MIN_BENCH_PCT) {
      ...
      if (benchPct >= STRONG_BENCH_PCT) {
        // ×¡×¤×¡×œ ×—×–×§ ×ž××•×“
        insight = `×¡×¤×¡×œ ×“×•×ž×™× × ×˜×™: ${startersPct}% ×ž×”×—×ž×™×©×™×™×”, ${benchPct}% ×ž×”×¡×¤×¡×œ`;
      } else {
        // ××™×–×•×Ÿ ×˜×•×‘
        insight = `××™×–×•×Ÿ ×˜×•×‘ ×‘×™×Ÿ ×—×ž×™×©×™×™×” (${startersPct}%) ×œ×¡×¤×¡×œ (${benchPct}%)`;
      }
      ...
      return {
        ...
        teamName,
        icon: 'âš–ï¸',
        text: `${teamName}: ${insight} â€“ ${startersPpg.toFixed(1)} vs ${benchPpg.toFixed(1)} × ×§' ×œ×ž×©×—×§`,
        textShort: `${startersPct}% ×—×ž×™×©×™×™×”, ${benchPct}% ×¡×¤×¡×œ`
      };
    }
  ...
  detectTurnoverCapitalization(teamName, teamData, allTeams) {
    ...
    if (pointsFromToPpg >= THRESHOLD) {
      ...
      const rankText = rank ? ` (×ž×§×•× ${rank} ×‘×œ×™×’×” ×‘× ×™×¦×•×œ ××™×‘×•×“×™×)` : '';
      ...
      return {
        ...
        rank,
        icon: 'ðŸ’°',
        text: `${teamName}${rankText} - ${actionText}! ${pointsFromToPpg} × ×§' ×œ×ž×©×—×§ ×ž××™×‘×•×“×™× ×©×œ ×”×™×¨×™×‘×”`,
        textShort: `${pointsFromToPpg} × ×§' ×ž××™×‘×•×“×™×`
      };
    }
    return null;
  }
  ...
  detectSecondChanceMasters(teamName, teamData, allTeams) {
    ...
    if (secondChancePpg >= THRESHOLD) {
      ...
      const rankText = rank ? ` (×ž×§×•× ${rank} ×‘×œ×™×’×” ×‘×”×–×“×ž× ×•×ª ×©× ×™×™×”)` : '';
      ...
      return {
        ...
        rank,
        icon: 'ðŸ”„',
        text: `${teamName}${rankText} - ${actionText}! ${secondChancePpg} × ×§' ×œ×ž×©×—×§ ×ž×”×–×“×ž× ×•×ª ×©× ×™×™×”`,
        textShort: `${secondChancePpg} × ×§' ×”×–×“×ž× ×•×ª 2`
      };
    }
    return null;
  }
  ...
  detectDefensiveWall(teamName, teamData, leagueAvgOppPpg, allTeams) {
    ...
    if (oppPpg < threshold) {
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('team', 'DEFENSIVE_WALL', {
        ...
        diff
      }) || `${teamName}${rankText} ×ž×’×™×¢×” ×¢× ×”×’× ×” ×ž×¦×•×™× ×ª â€“ ×”×™×¨×™×‘×•×ª ×©×œ×” ×¢×œ ${oppPpg.toFixed(1)} × ×§' ×œ×ž×©×—×§, ${diff} ×¤×—×•×ª ×ž×”×ž×ž×•×¦×¢`;
  ...
  detectReboundDominance(teamName, teamData, opponentData) {
    ...
    if (diff >= THRESHOLD) {
      ...
      }) || `×¢×œ ×”× ×™×™×¨, ${teamName} ×ž×’×™×¢×” ×¢× ×™×ª×¨×•×Ÿ ×‘×¨×•×¨ ×‘×¨×™×‘××•× ×“ â€“ ×¤×œ×•×¡ ${diff.toFixed(1)} ×›×“×•×¨×™× ×—×•×–×¨×™× ×‘×ž×ž×•×¦×¢ ×œ×¢×•×ž×ª ${opponentData.teamName}`;
  ...
  detectTurnoverCreators(teamName, teamData, leagueAvgSpg, allTeams) {
    ...
    if (spg >= leagueAvgSpg + THRESHOLD_DIFF) {
      ...
      return {
        ...
        icon: 'ðŸ•µï¸',
        text: `${teamName}${rankText} ${actionText} - ${spg.toFixed(1)} ×—×˜×™×¤×•×ª ×œ×ž×©×—×§ (${diff} ×ž×¢×œ ×ž×ž×•×¦×¢ ×”×œ×™×’×”)`,
        textShort: `${spg.toFixed(1)} ×—×˜×™×¤×•×ª ×œ×ž×©×—×§`
      };
    }
    return null;
  }
  ...
  detectBlockParty(teamName, games, teamData, allTeams) {
    ...
    if (bpg >= THRESHOLD) {
      ...
      if (topBlocker && maxBpg >= 1.5) {
        ...
        playerDetail = ` ×›×©-${playerName} ×ž×•×‘×™×œ ×¢× ${maxBpg.toFixed(1)} ×—×¡×™×ž×•×ª ×œ×ž×©×—×§`;
      }
      ...
      return {
        ...
        rank,
        icon: 'ðŸš«',
        text: `${teamName}${rankText} ${actionText} - ${bpg.toFixed(1)} ×—×¡×™×ž×•×ª ×œ×ž×©×—×§!${playerDetail}`,
        textShort: `${bpg.toFixed(1)} ×—×¡×™×ž×•×ª ×œ×ž×©×—×§`
      };
    }
    return null;
  }
  ...
  detectThreePointDefense(teamName, games, allTeams) {
    const MIN_GAMES = 5;
    const DIFF_THRESHOLD = 5; // ×”×¤×¨×© ×ž×”×ž×ž×•×¦×¢ ×”×œ×™×’×ª×™
    ...
    if (diff < -DIFF_THRESHOLD) {
      return {
        ...
        teamName,
        icon: 'ðŸ›¡ï¸',
        text: `${teamName} ×”×’× ×” ×ž×¦×•×™× ×ª ×¢×œ ×©×œ×•×©! ×™×¨×™×‘×•×ª ×§×•×œ×¢×•×ª ${opp3PPct.toFixed(1)}% (×ž×ž×•×¦×¢ ×œ×™×’×ª×™ ${league3PPct.toFixed(1)}%)`,
      ...
      return {
        ...
        teamName,
        icon: 'âš ï¸',
        text: `${teamName} × ×•×ª× ×ª ×™×•×ª×¨ ×ž×“×™ ×ž×©×œ×•×©! ×™×¨×™×‘×•×ª ×§×•×œ×¢×•×ª ${opp3PPct.toFixed(1)}% (×ž×ž×•×¦×¢ ×œ×™×’×ª×™ ${league3PPct.toFixed(1)}%)`,
  ...
  /**
   * ×–×™×”×•×™ ×ž×’×ž×” ×‘×”×¤×¨×© × ×§×•×“×•×ª (Point Differential Trend)
   */
  detectPointDiffTrend(teamName, games) {
    ...
    if (Math.abs(change) >= THRESHOLD) {
      ...
      return {
        ...
        improving,
        icon: improving ? 'ðŸ“ˆ' : 'ðŸ“‰',
        text: `${teamName} ${improving ? '×‘×ž×’×ž×ª ×¢×œ×™×™×”' : '×‘×ž×’×ž×ª ×™×¨×™×“×”'} - ×”×¤×¨×© × ×§×•×“×•×ª ×©×œ ${recentAvgDiff > 0 ? '+' : ''}${recentAvgDiff.toFixed(1)} ×‘-5 ××—×¨×•× ×™× (×œ×¢×•×ž×ª ${seasonAvgDiff > 0 ? '+' : ''}${seasonAvgDiff.toFixed(1)} ×¢×•× ×ª×™)`,
  ...
  detectScheduleStrength(teamName, games, standings) {
    ...
    if (diff >= 40) {
      ...
      // ×”×¡×‘×¨ ×ž×¤×•×¨×˜ ×™×•×ª×¨
      const topHalfExplanation = `×§×‘×•×¦×•×ª ×‘×—×¦×™ ×”×¢×œ×™×•×Ÿ ×©×œ ×”×˜×‘×œ×” (×ž×§×•×ž×•×ª 1-${midPoint})`;
      const bottomHalfExplanation = `×§×‘×•×¦×•×ª ×‘×—×¦×™ ×”×ª×—×ª×•×Ÿ (×ž×§×•×ž×•×ª ${midPoint + 1}+)`;
      ...
      return {
        ...
        icon: 'ðŸ“Š',
        text: better === 'top' ? 
          `${teamName} ×ž×•×›×™×—×” ××ª ×¢×¦×ž×” ×ž×•×œ ×”×’×“×•×œ×™×! ${vsTopWins} × ×™×¦×—×•× ×•×ª ×ž×ª×•×š ${vsTopTotal} ×ž×©×—×§×™× × ×’×“ ×—×¦×™ ×¢×œ×™×•×Ÿ (${topPct.toFixed(0)}%), ×œ×¢×•×ž×ª ${vsBottomWins} ×ž×ª×•×š ${vsBottomTotal} × ×’×“ ×—×¦×™ ×ª×—×ª×•×Ÿ (${bottomPct.toFixed(0)}%)` :
          `${teamName} ×ž× ×¦×—×ª ×—×œ×©×™× ××‘×œ × ×›×©×œ×ª ×ž×•×œ ×—×–×§×™× - ${vsBottomWins} × ×™×¦×—×•× ×•×ª ×ž×ª×•×š ${vsBottomTotal} ×ž×©×—×§×™× × ×’×“ ×—×¦×™ ×ª×—×ª×•×Ÿ (${bottomPct.toFixed(0)}%), ××‘×œ ×¨×§ ${vsTopWins} ×ž×ª×•×š ${vsTopTotal} × ×’×“ ×—×¦×™ ×¢×œ×™×•×Ÿ (${topPct.toFixed(0)}%)`,
  ...
  /**
   * ×–×™×”×•×™ First Half vs Second Half Season - ×”×©×•×•××ª ×ž×—×¦×™×•×ª ×¢×•× ×”
   */
  detectSeasonHalves(teamName, games) {
    ...
    if (Math.abs(change) >= CHANGE_THRESHOLD) {
      ...
      return {
        ...
        teamName,
        icon,
        text: `${teamName} ${trend} ×‘×ž×”×œ×š ×”×¢×•× ×”! ×ž×—×¦×™×ª ×¨××©×•× ×”: ${firstWinPct.toFixed(0)}% (${firstWins}/${firstHalf.length}) â†’ ×ž×—×¦×™×ª ×©× ×™×™×”: ${secondWinPct.toFixed(0)}% (${secondWins}/${secondHalf.length})`,
  ...
  analyzeH2HVenue(teamA, teamB, h2hGames) {
    ...
    if (Math.abs(homePct - awayPct) >= 40) {
      return {
        ...
        icon: 'ðŸ“',
        text: `×™×ª×¨×•×Ÿ ×ž×’×¨×© ×‘×ž×¤×’×©×™×: ${teamA} ${homeWins}-${homeGames.length - homeWins} ×‘×‘×™×ª, ${awayWins}-${awayGames.length - awayWins} ×‘×—×•×¥ × ×’×“ ${teamB}`,
  ...
  analyzeH2HMarginTrend(teamA, teamB, h2hGames) {
    ...
    if (Math.abs(change) >= 8) {
      ...
      return {
        ...
        icon: improving ? 'ðŸ“ˆ' : 'ðŸ“‰',
        text: improving ?
          `${teamA} ×ž×¦×ž×¦×ž×ª ××ª ×”×¤×¢×¨ ×ž×•×œ ${teamB} - ×ž×ž×•×¦×¢ ×©×œ ${secondAvg > 0 ? '+' : ''}${secondAvg.toFixed(1)} × ×§' ×‘×ž×¤×’×©×™× ××—×¨×•× ×™× ×œ×¢×•×ž×ª ${firstAvg > 0 ? '+' : ''}${firstAvg.toFixed(1)} ×‘×ž×¤×’×©×™× ×¨××©×•× ×™×` :
          `×”×¤×¢×¨ ×‘×™×Ÿ ${teamA} ×œ-${teamB} ×’×“×œ - ${firstAvg > 0 ? '+' : ''}${firstAvg.toFixed(1)} × ×§' ×‘×ž×¤×’×©×™× ×¨××©×•× ×™× ×™×¨×“ ×œ-${secondAvg > 0 ? '+' : ''}${secondAvg.toFixed(1)} ×‘×ž×¤×’×©×™× ××—×¨×•× ×™×`,
        textShort: `${improving ? '×ž×¦×ž×¦×ž×ª' : '×ž×¨×—×™×‘×”'} ×”×¤×¢×¨ ×‘×ž×¤×’×©×™×`
      };
    }
    return null;
  }
  ...
  detectH2HTopScorer(teamA, teamB, games, h2h) {
    ...
    if (topScorer) {
      ...
      return {
        ...
        playerName,
        icon: 'ðŸŽ¯',
        text: `${playerName} ×ž-${topScorer.teamName} ×©×•×œ×˜ ×‘×ž×¤×’×©×™×! ${maxPpg.toFixed(1)} × ×§' ×‘×ž×ž×•×¦×¢ ×‘×ž×¤×’×©×™× ×™×©×™×¨×™× (${topScorer.games} ×ž×©×—×§×™×)`,
        textShort: `${playerName}: ${maxPpg.toFixed(1)} × ×§' ×‘×ž×¤×’×©×™×`
      };
    }
    return null;
  }
  ...
  detectH2HFlip(teamA, teamB, games) {
    ...
    if (firstHalfWinsA >= firstThreshold && secondHalfWinsB >= secondThreshold) {
      return {
        ...
        teamA, teamB,
        icon: 'ðŸ”„',
        text: `×©×™× ×•×™ ×›×™×•×•×Ÿ ×‘×ž×¤×’×©×™×! ${teamA} ×©×œ×˜×” ×‘×ª×—×™×œ×” (${firstHalfWinsA}/${mid}), ××‘×œ ${teamB} ×–×•×›×” ×œ××—×¨×•× ×” (${secondHalfWinsB}/${secondHalf.length})`,
        textShort: `${teamB} ×”×¤×›×” ××ª ×”×ž×’×ž×”`
      };
      ...
      return {
        ...
        teamA, teamB,
        icon: 'ðŸ”„',
        text: `×©×™× ×•×™ ×›×™×•×•×Ÿ ×‘×ž×¤×’×©×™×! ${teamB} ×©×œ×˜×” ×‘×ª×—×™×œ×” (${firstHalfWinsB}/${mid}), ××‘×œ ${teamA} ×–×•×›×” ×œ××—×¨×•× ×” (${secondHalfWinsA}/${secondHalf.length})`,
        textShort: `${teamA} ×”×¤×›×” ××ª ×”×ž×’×ž×”`
      };
    }
    return null;
  }
  ...
  // âŒ detectFirstQuarterTeam ×”×•×¡×¨ - ×”×™×” ×›×¤×™×œ×•×ª ×¢× detectQuarterDominance

  /**
   * ×–×™×”×•×™ Slow Starters - ×ž×¤×’×¨×ª ×‘×ž×—×¦×™×ª ××‘×œ ×ž× ×¦×—×ª
   */
  detectSlowStarters(teamName, games) {
    ...
    if (comebackPct >= THRESHOLD) {
      return {
        ...
        icon: 'ðŸ¢',
        text: `${teamName} ×ž×ª×—×™×œ×” ×œ××˜ ××‘×œ ×ž×¡×™×™×ž×ª ×—×–×§ - ${comebackWins} × ×™×¦×—×•× ×•×ª ×ž×ª×•×š ${halftimeDeficits} ×ž×©×—×§×™× ×©×¤×™×’×¨×• ×‘×ž×—×¦×™×ª (${(comebackPct * 100).toFixed(0)}%)`,
        textShort: `${comebackWins}/${halftimeDeficits} ×§××ž×‘×§×™× ×ž×ž×—×¦×™×ª`
      };
    }
    return null;
  }
  ...
  detectComebackKings(teamName, games) {
    ...
    if (bigComebacks >= MIN_COMEBACKS) {
      ...
      }) || `${teamName} ×ž×’×™×¢×” ×¢× ×™×›×•×œ×ª ×§××ž×‘×§ ×—×¨×™×’×” â€“ ${bigComebacks} × ×™×¦×—×•× ×•×ª ××—×¨×™ ×¤×™×’×•×¨ ×©×œ ${DEFICIT_THRESHOLD}+ × ×§×•×“×•×ª`;
  ...
  detectFourthQuarterCollapse(teamName, games) {
    ...
    if (q4LossPct >= THRESHOLD) {
      ...
      const text = window.IBBAInsightTemplates?.getRandomText('team', 'FOURTH_QUARTER_COLLAPSE', {
        ...
        q4Losses,
        gamesWithQ4Data
      }) || `${teamName} ×ž×’×™×¢×” ×¢× ×¡×™×ž×Ÿ ×©××œ×” ×‘×¨×‘×¢ ×”×¡×™×•× â€“ ×”×¤×¡×™×“×” ${q4Losses} ×ž×ª×•×š ${gamesWithQ4Data} ×¨×‘×¢×™× ×¨×‘×™×¢×™×™× ×‘×¢×•× ×”`;
  ...
  detectBestQuarter(teamName, games) {
    ...
    if (bestQuarter) {
      ...
      }) || `×”×¨×‘×¢ ${quarterName} ×”×•× × ×§×•×“×ª ×—×•×–×§ ×©×œ ${teamName} ×‘×¢×•× ×” â€“ +${bestAvgDiff.toFixed(1)} × ×§' ×‘×ž×ž×•×¦×¢ ×‘×¨×‘×¢ ×”×–×”`;
  ...
  detectLeagueLeader(teamName, teamData, allTeams) {
    const categories = {
      ...
      apg: { label: '×‘××¡×™×¡×˜×™×', icon: 'ðŸ¤', ascending: false },
      spg: { label: '×‘×—×˜×™×¤×•×ª', icon: 'ðŸƒ', ascending: false },
      bpg: { label: '×‘×—×¡×™×ž×•×ª', icon: 'ðŸš«', ascending: false },
      oppPpg: { label: '×‘×”×’× ×”', icon: 'ðŸ›¡ï¸', ascending: true }
    };
    ...
    if (leaderCategories.length >= 2) {
      ...
      const leaderText = hasFirstPlace ? '×‘×™×Ÿ ×”×ž×•×‘×™×œ×•×ª ×‘×œ×™×’×”' : '×‘×ž×§×•×ž×•×ª ×”×¨××©×•× ×™× ×‘×œ×™×’×”';
      ...
      return {
        ...
        teamName,
        icon: 'ðŸ†',
        text: `${teamName} ${leaderText}! ${topCat.icon} ×ž×§×•× ${topCat.rank} ${topCat.label} (${topCat.value}), ${secondCat.icon} ×ž×§×•× ${secondCat.rank} ${secondCat.label} (${secondCat.value})`,
  ...
  /**
   * ×–×™×”×•×™ Above/Below Average - ×ž×¢×œ/×ž×ª×—×ª ×œ×ž×ž×•×¦×¢ ×”×œ×™×’×”
   */
  detectAboveBelowAverage(teamName, teamData, allTeams) {
    ...
    if (aboveCategories.length >= 3) {
      ...
      return {
        ...
        teamName,
        icon: 'â­',
        text: `${teamName} ×ž×¢×œ ×”×ž×ž×•×¦×¢! ${categoriesList}`,
        textShort: `×ž×¢×œ ×ž×ž×•×¦×¢ ×‘-${aboveCategories.length} ×§×˜×’×•×¨×™×•×ª`
      };
      ...
      return {
        ...
        teamName,
        icon: 'ðŸ“‰',
        text: `${teamName} ×ž×ª×—×ª ×œ×ž×ž×•×¦×¢: ${categoriesList}`,
        textShort: `×ž×ª×—×ª ×œ×ž×ž×•×¦×¢ ×‘-${belowCategories.length} ×§×˜×’×•×¨×™×•×ª`
      };
    }
    return null;
  }
  ...
  generateMatchupInsights(teamA, teamB, reportData) {
    ...
    // ×¨×§ ×× ××™×Ÿ LEAGUE_LEADER - ×”×¦×’ ABOVE_BELOW_AVERAGE (×œ×ž× ×•×¢ ×›×¤×™×œ×•×ª)
    if (!leagueLeaderA) {
  ...
  /**
   * ××™×–×•×Ÿ Insights ×‘×™×Ÿ ×§×‘×•×¦×•×ª - ×ž×•×•×“× ×©×œ×©×ª×™ ×”×§×‘×•×¦×•×ª ×™×© representation
   */
  balanceTeamInsights(insights, teamA, teamB, teamAData, teamBData, allTeams) {
    const MIN_INSIGHTS_PER_TEAM = 5; // ×œ×¤×—×•×ª 5 insights ×œ×›×œ ×§×‘×•×¦×” (×”×•×¢×œ×” ×ž-3)
    const MAX_RATIO = 2; // ×™×—×¡ ×ž×§×¡×™×ž×œ×™ ×‘×™×Ÿ ×§×‘×•×¦×•×ª (2:1)
